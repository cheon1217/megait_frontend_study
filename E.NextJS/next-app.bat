...
echo 1. 프로젝트를 생성합니다.
echo ----------------------------------
echo $ yarn create next-app "%1"
call yarn create next-app "%1"
echo.
...
echo ----------------------------------
echo .yarnrc.yml과 package.json, next.config.js 파일을 수정합니다.
...
powershell -Command "(gc next.config.js) -replace 'reactStrictMode: true,', 'compiler: { styledComponents: true }' | Out-File -encoding utf8 next.config.js"
