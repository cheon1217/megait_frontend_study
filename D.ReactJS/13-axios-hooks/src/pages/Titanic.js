...
import { SexLabel, EmbarkedLable, SurvivedLabel, PclassLabel } from "../components/Labels";
...
const Titanic = memo(() => {
    ...
    // --> 기본적으로 컴폰넌트의 마운트와 동시에 자동 실행되어 응답 결과를 data에 저장한다.
    ...
    return (
        <div>
            ...
            <SelectContainer>
                ...
                <select name="pclass" onChange={onSelectChange}>
                    <option value="">-- 객실등급 선택 --</option>
                    ...
                    {data && data.map(({
                        ...
                        return (
                            <tr key={id}>
                                ...
                                <td>{age}</td>
                                <td>{sibsp + parch}</td>
                                <td><PclassLabel $pclass={pclass} />등석</td>
                                <td>{cabin}</td>
                                <td>{ticket}</td>
                                <td>{fare}</td>
                                <td><EmbarkedLable $embarked={embarked} /></td>
                                <td><SurvivedLabel $survived={survived.toString()} /></td>
                            </tr>
                        );
                    })}
                </tbody>
