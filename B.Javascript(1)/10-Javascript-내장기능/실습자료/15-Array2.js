...
const found1 = arr6.find(v => v % 2 == 0);
console.log(found1);
...
const d3 = arr9.filter(function (v, i, arr) {
    ...
    if (v % 2 == 0) {
        ...
        // true를 리턴하더라도 배열의 모든 원소를 팀색하기 전까지는 종료되지 않는다.
        return true;
    } else {
        // false가 리턴되는 경우 v는 버려진다.
        return false;
 ...
 * index : 이번 회차에 탐색되는 배열 원소의 인덱스 (생략 가능)
 ...
 */
const result = arr12.reduce((accumulator, currentValue, index) => {
...
const result4 = arr12.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
...
// 전체 확진자 수 구하기
// 객체를 탐색할 때는 accumulator의 초기값을 설정하고 0번째 원소부터 currentValue로 받아야한다.
