...
// 삭제할 원소의 수를 0으로 저장하면 중간 삽입 효과가 있음
// 기존의 원소들은 뒤로 밀려남
data.splice(3, 0, 'd', 'e');
console.log(data);

// a에 b와 c를 결합한 새로운 배열 반환
const a = [1, 2];
const b = [3, 4, 5];
const c = [6, 7, 8, 9];
...
// arr.lastIndecOf(item, from)는 위 메서드와 동일한 기능을 하는데, 검색을 끝에서부터 시작한다.
...
const arr2 = [NaN];
console.log(arr2.indexOf(NaN)); // -1 (완전 항등 비교 === 는 NaN엔 동작히지 않으므로 0이 출력되지 않는다.)
console.log(arr2.includes(NaN)); // true (NaN의 여부를 확인);